...
mqttClient.on('connect', () => {
  ...
  // Subscribe to topics for each specified device
...
mqttClient.on('message', (topic, message) => {
  try {
    ...
    // Find the device associated with the received topic
    const device = devices.find((d) => topic.includes(d.id));
    if (!device) {
      console.log('Ignoring message from unknown topic:', topic);
      return;
    }

    console.log(`Received message from ${device.name} with device ID ${device.id}`);
